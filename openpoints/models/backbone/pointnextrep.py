...
class PointNextRepEncoder(nn.Module):
...
@MODELS.register_module()
class AllConcatDecoder(nn.Module):
    def __init__(self,
        ...
        for i in range(-1, -n_decoder_stages - 1, -1):
            transize[i] = self._make_TranSize(encoder_channel_list[i], min(encoder_channel_list[i], 64))
        self.transize = nn.Sequential(*transize)
        convs_fuse = []
        mlp_fuse = [32+64*4, 64, self.out_channels]
...
@MODELS.register_module()
class AllConcatDecoder128(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
                 decoder_stages: int = 4, 
        ...
        self.out_channels = fp_channels[-n_decoder_stages]
        for i in range(-1, -n_decoder_stages - 1, -1):
            transize[i] = self._make_TranSize(encoder_channel_list[i], min(encoder_channel_list[i], 128))
        self.transize = nn.Sequential(*transize)
        convs_fuse = []
        mlp_fuse = [32+64+128*3, 64, self.out_channels]
        for i in range(len(mlp_fuse) - 1):
            convs_fuse.append(create_convblock1d(mlp_fuse[i], mlp_fuse[i + 1],
                                            norm_args=norm_args, act_args=act_args
                                            ))
...
@MODELS.register_module()
class AllConcatDecoder256(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
                 decoder_stages: int = 4, 
...
@MODELS.register_module()
class AllConcatDecoderAll256(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
                 decoder_stages: int = 4, 
        ...
        self.out_channels = fp_channels[-n_decoder_stages]
        for i in range(-1, -n_decoder_stages - 1, -1):
            transize[i] = self._make_TranSize(encoder_channel_list[i], 256)
            # transize[i] = self._make_TranSize(encoder_channel_list[i], min(encoder_channel_list[i], encoder_channel_list[-2]))
        self.transize = nn.Sequential(*transize)
        convs_fuse = []
        mlp_fuse = [32+256*4, 64, self.out_channels]
        # mlp_fuse = [sum(encoder_channel_list[:-1])+encoder_channel_list[-2], 2*self.out_channels, self.out_channels]
        for i in range(len(mlp_fuse) - 1):
            convs_fuse.append(create_convblock1d(mlp_fuse[i], mlp_fuse[i + 1],
                                            norm_args=norm_args, act_args=act_args
...
@MODELS.register_module()
class AllConcatDecoderAll128(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
                 decoder_stages: int = 4, 
        ...
        self.out_channels = fp_channels[-n_decoder_stages]
        for i in range(-1, -n_decoder_stages - 1, -1):
            transize[i] = self._make_TranSize(encoder_channel_list[i], 128)
            # transize[i] = self._make_TranSize(encoder_channel_list[i], min(encoder_channel_list[i], encoder_channel_list[-2]))
        self.transize = nn.Sequential(*transize)
        convs_fuse = []
        mlp_fuse = [32+128*4, 64, self.out_channels]
        # mlp_fuse = [sum(encoder_channel_list[:-1])+encoder_channel_list[-2], 2*self.out_channels, self.out_channels]
        for i in range(len(mlp_fuse) - 1):
            convs_fuse.append(create_convblock1d(mlp_fuse[i], mlp_fuse[i + 1],
                                            norm_args=norm_args, act_args=act_args
...
class PyramidPoolAgg(nn.Module):
    ...
    def pool_repeat(self, f, p, i):
        repeat = 4-i
        for k in range(repeat):
            _, f_j = self.grouper(p[i+1+k], p[i+k], f)
            f = self.pool(f_j) 
        return f
    ...
    def forward(self, p, f):
        ...
        for i, fi in enumerate(f):
            pool_f = self.pool_repeat(fi, p, i)
            pool_feats.append(pool_f)
        return torch.cat(pool_feats, dim=1)
...
class InjectionMultiSum(nn.Module):
    def __init__(self,
        ...
        self.local_embedding = create_convblock1d(inp, oup, norm_args=norm_args, act_args=None)
        self.global_embedding = create_convblock1d(inp, oup, norm_args=norm_args, act_args=None)
        self.global_act = create_convblock1d(inp, oup, norm_args=norm_args, act_args=None)
        self.act = h_sigmoid()

    def interpolate_repeat(self, f, p, i):
        repeat = 4-i
        for k in range(repeat):
            f = three_interpolation(p[-1-k-1], p[-1-k], f)
        return f

    def forward(self, x_l, x_g, p, i):
        '''
        x_g: global features
        x_l: local features
        '''
        ...
        sig_act = three_interpolation(p[i], p[-1], sig_act)
        global_feat = self.global_embedding(x_g)
        global_feat = three_interpolation(p[i], p[-1], global_feat)
        # global_feat = self.interpolate_repeat(global_feat, p, i)
        out = local_feat * sig_act + global_feat
        return out


class AllConcatSum(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
                 decoder_stages: int = 4, 
        ...
        convs_fuse = []
        if merge=='concat':
            mlp_fuse = [256*5, 64, self.out_channels]
        elif merge=='sum':
            mlp_fuse = [256, 64, self.out_channels]
        else:
    ...
    def forward(self, p, f):
        for i in range(-1, -len(self.encoder_channel_list), -1):
            f[i] = three_interpolation(p[0], p[i], f[i])
        if self.merge=='concat':
            f_merge = torch.cat(f, dim=1)
        elif self.merge=='sum':
            f_merge = sum(f)
        else:
            raise NotImplementedError
        f_out = self.convs_fuse(f_merge)        
        return f_out
...
class SIMDecoder(nn.Module):
    def __init__(self,
        ...
        n_decoder_stages = len(fp_channels)
        self.out_channels = fp_channels[-n_decoder_stages]
        ## pool each level to the lowest resolution
        self.ppa = PyramidPoolAgg()
        ## fuse each level into one global semantic
        convs_fuse = []
        mlp_fuse = [sum(encoder_channel_list), sum(encoder_channel_list), sum(encoder_channel_list)]
        for i in range(len(mlp_fuse) - 1):
            convs_fuse.append(create_convblock1d(mlp_fuse[i], mlp_fuse[i + 1],
                                            norm_args=norm_args, act_args=act_args
                                            ))
        ...
        sim_out_channel_list = [256, 256, 256, 256, 256]
        self.SIM = nn.ModuleList()
        for i in range(len(encoder_channel_list)):
                self.SIM.append(
                    InjectionMultiSum(encoder_channel_list[i], sim_out_channel_list[i], norm_args=norm_args, act_args=act_args))
        ## allConcatSum
        self.allConcatSum = AllConcatSum(encoder_channel_list=encoder_channel_list)

    def forward(self, p, f):
        p, f = p[1:], f[1:]
        pool_f = self.ppa(p, f)
        fuse_pool_f = self.convs_fuse(pool_f)
        xx = fuse_pool_f.split(self.encoder_channel_list, dim=1)
        results = []
        for i in range(len(self.encoder_channel_list)):
            local_tokens = f[i]
            global_semantics = xx[i]
            out_ = self.SIM[i](local_tokens, global_semantics, p, i)
            results.append(out_)
        f_out = self.allConcatSum(p, results)
        return f_out


@MODELS.register_module()
class AllConcatDecoderNoMLP(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
                 decoder_stages: int = 4, 
        ...
        self.out_channels = fp_channels[-n_decoder_stages]
        convs_fuse = []
        mlp_fuse = [32+64+128+256+512, 64, self.out_channels]
        for i in range(len(mlp_fuse) - 1):
            convs_fuse.append(create_convblock1d(mlp_fuse[i], mlp_fuse[i + 1],
                                            norm_args=norm_args, act_args=act_args
                                            ))
    ...
    def forward(self, p, f):
        for i in range(-1, -len(f[1:]), -1):
            f[i] = three_interpolation(p[1], p[i], f[i])
        f_cat = torch.cat(f[1:], dim=1)
        f_out = self.convs_fuse(f_cat)        
        return f_out
...
@MODELS.register_module()
class All4ConcatDecoder256(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
                 decoder_stages: int = 4, 
        ...
        self.transize = nn.Sequential(*transize)
        convs_fuse = []
        # mlp_fuse = [64+128+256*2, 64, self.out_channels]
        mlp_fuse = [sum(encoder_channel_list[1:-1])+encoder_channel_list[-2], 2*self.out_channels, self.out_channels]
        for i in range(len(mlp_fuse) - 1):
            convs_fuse.append(create_convblock1d(mlp_fuse[i], mlp_fuse[i + 1],
                                            norm_args=norm_args, act_args=act_args
                                            ))
        self.convs_fuse = nn.Sequential(*convs_fuse)
        mlp = [self.out_channels+self.out_channels, self.out_channels]
...
@MODELS.register_module()
class All4ConcatDecoder256_64(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
                 decoder_stages: int = 4, 
...
@MODELS.register_module()
class All3ConcatDecoder256_64(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
                 decoder_stages: int = 4, 
