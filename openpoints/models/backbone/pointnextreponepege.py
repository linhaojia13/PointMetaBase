...
class SetAbstraction(nn.Module):
    ...
    def forward(self, pf_pe_dp0):
        p, f, pe, dp0 = pf_pe_dp0
        new_dp0 = None
        if self.is_head:
            f = self.convs(f)  # (n, c)
        else:
            if not self.all_aggr:
                ...
                new_dp0 = torch.gather(dp0, 2, idx.unsqueeze(1).unsqueeze(-1).expand(-1, 3, -1, 32))
                # dp0:(B, 3, N, 32), idx:(B, N)
            else:
                new_p = p
                new_dp0 = dp0
            """ DEBUG neighbor numbers. 
...
class PointNextRepOnePEGEEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.ge_grouper = create_grouper(group_args)
    ...
    def _make_ge_enc(self, block, channels, blocks, stride, group_args, is_head=False):
        ...
        channels2 = [3, 16, channels] #[3, channels]
        convs2 = []
        for i in range(len(channels2) - 1):  # #layers in each blocks
            convs2.append(create_convblock2d(channels2[i], channels2[i + 1],
                                            norm_args=self.norm_args,
    ...
    def forward_seg_feat(self, p0, f0=None):
        ...
        if f0 is None:
            f0 = p0.clone().transpose(1, 2).contiguous()
        dp0, _ = self.ge_grouper(p0, p0, None) #(B, 3, N, K)
        p, f = [p0], [f0]
        for i in range(0, len(self.encoder)):
            if i == 0:
                pe = None
                _p, _f, _, _ = self.encoder[i]([p[-1], f[-1], pe, dp0])
            else:
                _p, _f, _, dp0 = self.encoder[i][0]([p[-1], f[-1], pe, dp0]) # dp0 is downsampled in SA
                # grouping for pe
                dp, _ = self.pe_grouper[i](_p, _p, None)
                # conv on neighborhood_dp for pe
                pe = self.pe_encoder[i](dp)
                # conv on neighborhood_dp0 for ge
                ge = self.ge_encoder[i](dp0)
                # pool for ge
                ge = torch.max(ge, dim=-1, keepdim=False)[0]                 
