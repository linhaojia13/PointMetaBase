...
class LocalAggregation(nn.Module):
    ...
    def forward(self, pf, pe, ge) -> torch.Tensor:
...
class SetAbstraction(nn.Module):
    ...
    def forward(self, pf_pe_geinp):
        p, f, pe, geinp = pf_pe_geinp
        new_geinp = None
        if self.is_head:
            f = self.convs(f)  # (n, c)
        else:
            if not self.all_aggr:
                ...
                new_geinp = torch.gather(geinp, 2, idx.unsqueeze(1).expand(-1, geinp.shape[1], -1))
                # geinp:(B, 16, N), idx:(B, N)
            else:
                new_p = p
                new_geinp = geinp
            """ DEBUG neighbor numbers. 
...
class InvResMLP(nn.Module):
    ...
    def forward(self, pf_pe_ge):
        p, f, pe, ge = pf_pe_ge
        identity = f
        f = self.convs([p, f], pe, ge)
        f = self.pwconv(f)
...
class PointNextRepOnePEHGEEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(blocks)):
            ...
            ge_encoder.append(self._make_ge_enc(
                block, channels[i], blocks[i], stride=strides[i], group_args=group_args,
                is_head=i == 0 and strides[i] == 1
            ))
        ...
        self.ge_encoder = nn.Sequential(*ge_encoder)
    ...
    def _make_ge_enc(self, block, channels, blocks, stride, group_args, is_head=False):
        ## for GE of this stage
        channels2 = [16, 16, channels] #[16, channels]
        convs2 = []
        for i in range(len(channels2) - 1):  # #layers in each blocks
            convs2.append(create_convblock1d(channels2[i], channels2[i + 1],
                                            norm_args=self.norm_args,
                                            act_args=self.act_args,
                                            **self.conv_args)
                         )
    ...
    def forward_seg_feat(self, p0, f0=None):
        ...
        h = f0[:,-1:,:] # (B, 1, N)
        emb_dim = 16
        ge_inp = get_h_emb(h, emb_dim)
        p, f = [p0], [f0]
        for i in range(0, len(self.encoder)):
            if i == 0:
                pe = None
                _p, _f, _, _ = self.encoder[i]([p[-1], f[-1], pe, ge_inp])
            else:
                _p, _f, _, ge_inp = self.encoder[i][0]([p[-1], f[-1], pe, ge_inp]) # dp0 is downsampled in SA
                # grouping for pe
                dp, _ = self.pe_grouper[i](_p, _p, None)
                # conv on neighborhood_dp for pe
                pe = self.pe_encoder[i](dp)
                # conv on ge_inp for ge
                if i < 3: #2 3
                    ge = self.ge_encoder[i](ge_inp)  
                else:
                    ge = None              
                _p, _f, _, _ = self.encoder[i][1:]([_p, _f, pe, ge])
            p.append(_p)
            f.append(_f)
        return p, f
...
def get_h_emb(h, emb_dim=16):
    B, _, N = h.shape
    mid_dim = emb_dim//2
    wave_length = 10 #10 #1000
    alpha = 1 #1 #100
    feat_range = torch.arange(mid_dim, dtype=torch.float32).cuda()  # (mid_dim, )
    dim_mat = torch.pow(1.0 * wave_length, (1.0 / mid_dim) * feat_range)  # (mid_dim, )
    position_mat = torch.unsqueeze(alpha * h, -1)  # (B, 1, N, 1)
    div_mat = torch.div(position_mat, dim_mat)  # (B, 1, N, mid_dim)
    sin_mat = torch.sin(div_mat)  # (B, 1, N, mid_dim)
    cos_mat = torch.cos(div_mat)  # (B, 1, N, mid_dim)
    position_embedding = torch.cat([sin_mat, cos_mat], -1)  # (B, 1, N, 2*mid_dim)
    position_embedding = position_embedding.permute(0, 1, 3, 2).contiguous()
    position_embedding = position_embedding.view(B, emb_dim, N)  # (B, C, N)   
