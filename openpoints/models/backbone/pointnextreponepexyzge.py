    ...
    def forward(self, pf_pe_geinp):
        ...
        if self.is_head:
            ...
            if not self.all_aggr:
                ...
                new_geinp = torch.gather(geinp, 2, idx.unsqueeze(1).expand(-1, 3, -1))
                # geinp:(B, 4, N), idx:(B, N)
            else:
                new_p = p
                new_geinp = geinp
...
class PointNextRepOnePEXYZGEEncoder(nn.Module):
    ...
    def _make_ge_enc(self, block, channels, blocks, stride, group_args, is_head=False):
        ...
        channels2 = [3, 16, channels] #[3, 16, channels] #[3, channels]
        convs2 = []
        for i in range(len(channels2) - 1):  # #layers in each blocks
    ...
    def forward_seg_feat(self, p0, f0=None):
        ...
        if f0 is None:
            f0 = p0.clone().transpose(1, 2).contiguous()
        ge_inp = p0.clone().transpose(1, 2).contiguous()
        p, f = [p0], [f0]
        for i in range(0, len(self.encoder)):
            if i == 0:
                pe = None
