...
class TRBlock1(nn.Module):
    def __init__(self,
                 in_channels,
                 norm_args=None,
                 act_args=None,
                 att_args={'att_type': 'Attention', "num_heads": 4},
                 aggr_args={'feature_type': 'dp_fj', "reduction": 'max'},
                 group_args={'NAME': 'ballquery'},
                 conv_args=None,
                 expansion=1,
...
class TRBlock(nn.Module):
    def __init__(self,
        ...
        self.q_conv = nn.Conv1d(in_channels, in_channels // 4, 1, bias=False)
        self.k_conv = nn.Conv1d(in_channels, in_channels // 4, 1, bias=False)
...
class PointNextRepTREncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(blocks)):
            ...
            encoder.append(self._make_enc(
                block, channels[i], blocks[i], tr_blocks[i], stride=strides[i], group_args=group_args,
    ...
    def _make_enc(self, block, channels, blocks, tr_blocks, stride, group_args, is_head=False):
        layers = []
        ...
        for i in range(0, tr_blocks):
            layers.append(TRBlock(self.in_channels,
                                aggr_args=self.aggr_args,
                                norm_args=self.norm_args, act_args=self.act_args, group_args=group_args,
                                conv_args=self.conv_args, expansion=self.expansion,
                                use_res=self.use_res
